...
import { BellRing, CheckCircle2, Clock3, Pencil, Trash2, XCircle } from 'lucide-react';
...
export default function FinancePlanList({ plans = [], todayKey, language = 'ru', isLight, onResolve, onEdit, onDelete, busyId }) {
  ...
  return <section className={`overflow-hidden rounded-2xl border ${panel}`}>
    ...
    <div className="divide-y divide-zinc-500/10">
      {plans.map((plan) => {
        ...
        const paymentCounter = plan.repeat_total ? `${plan.repeat_index || 1} ${copy.of} ${plan.repeat_total}` : '';
        const status = completed ? copy.confirmed : missed ? copy.missed : overdue ? copy.overdue : active && due ? copy.waiting : copy.planned;
        const tone = completed ? 'emerald' : missed || overdue ? 'rose' : plan.kind === 'income' ? 'emerald' : 'rose';
        return <div key={plan.id} className={`px-4 py-3.5 ${isLight ? (tone === 'emerald' ? 'bg-emerald-50/30' : 'bg-rose-50/25') : (tone === 'emerald' ? 'bg-emerald-400/[0.025]' : 'bg-rose-400/[0.025]')}`}>
          <div className="flex items-start gap-3">
            <span className={`mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-xl ${completed ? 'bg-emerald-500/10 text-emerald-500' : missed || overdue ? 'bg-rose-500/10 text-rose-500' : plan.kind === 'income' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>{completed ? <CheckCircle2 className="h-4 w-4" /> : missed ? <XCircle className="h-4 w-4" /> : <Clock3 className="h-4 w-4" />}</span>
            <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="min-w-0 break-words text-sm font-semibold leading-snug">{plan.title}</p><span className={`shrink-0 font-data text-sm font-semibold ${plan.amount == null ? 'text-zinc-500' : plan.kind === 'income' ? 'text-emerald-500' : 'text-rose-500'}`}>{amount(plan, localeCode, copy)}</span></div><p className="mt-1 text-xs text-zinc-500">{planTime(plan)} {repeat} {paymentCounter ? ` · ${paymentCounter}` : ''} {remind} · {status}{plan.repeat_until ? ` · ${copy.until} ${endDateLabel(plan.repeat_until, localeCode)}` : ''}</p></div>
          </div>
          {active && <div className={`mt-3 flex flex-wrap items-center gap-2 ${due ? 'justify-between' : 'justify-end'}`}>
            <div className="flex items-center gap-2"><button type="button" disabled={busyId === actionPlan.id} onClick={() => onEdit(actionPlan)} className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold transition ${isLight ? 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:text-slate-900' : 'border-white/[0.08] bg-white/[0.03] text-zinc-400 hover:border-white/[0.16] hover:text-zinc-100'} disabled:opacity-50`}><Pencil className="h-3.5 w-3.5" />{plan.amount == null ? copy.addAmount : copy.edit}</button>
            <button type="button" disabled={busyId === actionPlan.id} onClick={() => onDelete?.(actionPlan)} className={`grid h-9 w-9 place-items-center rounded-xl border transition ${isLight ? 'border-slate-200 bg-white text-slate-400 hover:border-rose-300 hover:text-rose-500' : 'border-white/[0.08] bg-white/[0.03] text-zinc-500 hover:border-rose-400/40 hover:text-rose-400'} disabled:opacity-50`} aria-label={`${copy.deletePlan}: ${plan.title}`} title={copy.deletePlan}><Trash2 className="h-3.5 w-3.5" /></button></div>
