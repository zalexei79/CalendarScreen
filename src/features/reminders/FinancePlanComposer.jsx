    ...
    <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/70 p-3 backdrop-blur-sm sm:items-center" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form onSubmit={submit} className={`w-full max-w-[430px] overflow-hidden rounded-[30px] border shadow-[0_28px_90px_rgba(0,0,0,.55)] ${panel}`}>
        ...
        <div className="max-h-[min(70dvh,610px)] space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
          ...
          <section><div className="mb-2 flex items-center justify-between"><p className={`font-data text-[10px] uppercase tracking-[0.18em] ${muted}`}>03 · Расписание</p><span className={`text-[11px] ${muted}`}>{repeatSummary}</span></div><div className={`divide-y rounded-2xl border ${isLight ? 'divide-slate-100 border-slate-200 bg-white' : 'divide-white/[0.06] border-white/[0.09] bg-black/15'}`}><label className="flex items-center gap-3 px-4 py-3.5"><Repeat2 className="h-4 w-4 shrink-0 text-zinc-400" /><span className="min-w-0 flex-1"><span className="block text-sm font-medium">Повторять</span><span className={`mt-0.5 block text-[11px] ${muted}`}>{repeatSummary}</span></span><SelectValue value={repeatRule} onChange={setRepeatRule} options={[['none', 'Один раз'], ['weekly', 'Каждую неделю'], ['monthly', `Каждый месяц · ${dayNumber} число`], ['yearly', 'Каждый год']]} className={selectTone} /></label><label className="flex items-center gap-3 px-4 py-3.5"><BellRing className="h-4 w-4 shrink-0 text-zinc-400" /><span className="min-w-0 flex-1"><span className="block text-sm font-medium">Напомнить</span><span className={`mt-0.5 block text-[11px] ${muted}`}>{remindOffset === 'at_time' ? 'в момент события' : `заранее · ${remindOffset === '1_day' ? '1 день' : remindOffset === '3_days' ? '3 дня' : '1 неделя'}`}</span></span><SelectValue value={remindOffset} onChange={setRemindOffset} options={[['at_time', 'В момент'], ['1_day', 'За 1 день'], ['3_days', 'За 3 дня'], ['1_week', 'За неделю']]} className={selectTone} /></label><label className="flex items-center gap-3 px-4 py-3.5"><CalendarClock className="h-4 w-4 shrink-0 text-zinc-400" /><span className="min-w-0 flex-1"><span className="block text-sm font-medium">Время события</span><span className={`mt-0.5 block text-[11px] ${muted}`}>по местному времени</span></span><input required type="time" value={time} onChange={(event) => setTime(event.target.value)} className={`bg-transparent font-data text-sm outline-none ${selectTone}`} /></label></div>{repeatRule === 'monthly' && <p className={`mt-2 rounded-xl px-3 py-2.5 text-xs ${quiet}`}>DAYRIS будет напоминать <span className="font-semibold">каждого {dayNumber} числа</span> — без создания записей на годы вперёд.</p>}{reminderTooLate && <p role="alert" className="mt-2 rounded-xl bg-amber-400/10 px-3 py-2.5 text-xs leading-relaxed text-amber-500">Раннее окно для ближайшего события уже прошло. Первое уведомление придёт в момент события, а следующие — по выбранному правилу.</p>}</section>

          <p className={`rounded-2xl px-3.5 py-3 text-xs leading-relaxed ${quiet}`}>План появится в календаре сразу. В баланс и статистику он попадёт только после подтверждения.</p>{error && <p role="alert" className={`rounded-xl border px-3 py-2 text-xs leading-relaxed ${errorTone}`}>{String(error).includes('TIME_OUT_OF_RANGE') ? 'Событие должно быть в будущем. Для ближайшего события выбери более позднюю дату или время.' : String(error).includes('reminders_check') ? 'Сумму можно добавить позже — напоминание сохранится без неё.' : error}</p>}<button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-400 px-4 py-3.5 text-sm font-bold text-zinc-950 shadow-lg shadow-amber-500/15 transition hover:bg-amber-300 active:scale-[.99] disabled:cursor-not-allowed disabled:opacity-45">{busy ? 'Сохраняю…' : <><Check className="h-4 w-4" />Создать напоминание</>}</button>
        </div>
      </form>
    </div>
  );
