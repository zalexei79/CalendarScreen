...
const COPY = {
  ru: {
    ...
    saving: 'Сохраняю…', save: 'Сохранить изменения', create: 'Создать напоминание', createNow: 'Создать сейчас', details: 'Настроить детали', detailsTitle: 'Детали', next: 'Дальше', back: 'Назад', later: 'Сумму можно добавить позже — напоминание сохранится без неё.', future: 'Событие должно быть в будущем. Для ближайшего события выбери более позднюю дату или время.',
  ...
  en: {
    ...
    saving: 'Saving…', save: 'Save changes', create: 'Create reminder', createNow: 'Create now', details: 'Set details', detailsTitle: 'Details', next: 'Next', back: 'Back', later: 'You can add the amount later — the reminder will still be saved.', future: 'The event must be in the future. Choose a later date or time.',
  ...
  ro: {
    ...
    saving: 'Se salvează…', save: 'Salvează modificările', create: 'Creează memento', createNow: 'Creează acum', details: 'Configurează detaliile', detailsTitle: 'Detalii', next: 'Înainte', back: 'Înapoi', later: 'Poți adăuga suma mai târziu — mementoul va fi salvat.', future: 'Evenimentul trebuie să fie în viitor. Alege o dată sau o oră mai târzie.',
...
function PaymentPicker({ value, onChange, isLight, copy }) {
  ...
  return <span className="relative shrink-0">
    <select
      ...
      className={`h-10 min-w-[128px] appearance-none rounded-lg border-0 py-0 pl-3 pr-8 text-base font-semibold outline-none sm:text-sm ${
        isLight ? 'bg-slate-100 text-slate-900' : 'bg-white/[0.08] text-zinc-100'
      }`}
    >
      <option value="">{copy.countPlaceholder}</option>
      {Array.from({ length: maximum }, (_, index) => index + 1).map((item) => <option key={item} value={item}>{item}</option>)}
    </select>
    <ChevronDown className={`pointer-events-none absolute right-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 ${isLight ? 'text-slate-400' : 'text-zinc-500'}`} />
  </span>;
}
    ...
    <div className="fixed inset-0 z-[90] flex items-stretch justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-3" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form onSubmit={submit} className={`flex h-[100dvh] w-full max-w-[460px] flex-col overflow-hidden rounded-none border shadow-[0_28px_90px_rgba(0,0,0,.55)] sm:h-auto sm:rounded-[28px] ${panel}`}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:max-h-[min(72dvh,560px)] sm:flex-none sm:px-6 sm:py-5">
          <div className="mb-4 flex items-center gap-1.5" aria-label={`${step} / 3`}>
            {[1, 2, 3].map((item) => <span key={item} className={`h-1.5 flex-1 rounded-full transition-colors ${item <= step ? 'bg-amber-400' : isLight ? 'bg-slate-200' : 'bg-white/[0.08]'}`} />)}
          </div>
          ...
          {step === 2 && <section>
            <div className="mb-2 flex items-center justify-between"><p className={`font-data text-[10px] font-semibold uppercase tracking-[0.18em] ${isLight ? 'text-slate-700' : 'text-zinc-300'}`}>02 · {copy.schedule}</p><span className={`text-[11px] ${muted}`}>{repeatSummary}</span></div>
          ...
          {step === 3 && <section>
            <div className="mb-2 flex items-center justify-between"><p className={`font-data text-[10px] font-semibold uppercase tracking-[0.18em] ${isLight ? 'text-slate-700' : 'text-zinc-300'}`}>03 · {copy.detailsTitle}</p><span className={`text-[11px] ${muted}`}>{copy.amountHint}</span></div>
            ...
            <div className={`mt-3 flex items-center gap-3 rounded-xl border px-3.5 py-3 ${isLight ? 'border-slate-200 bg-white' : 'border-white/[0.09] bg-black/15'}`}><BellRing className="h-4 w-4 shrink-0 text-zinc-400" /><span className="min-w-0 flex-1"><span className="block text-sm font-medium">{copy.notify}</span><span className={`mt-0.5 block text-[10px] ${muted}`}>{remindSummary}</span></span><SelectValue value={remindOffset} onChange={setRemindOffset} isLight={isLight} options={[['at_time', copy.atTime], ['1_day', copy.before(resolveLanguage(language) === 'en' ? '1 day' : resolveLanguage(language) === 'ro' ? '1 zi' : '1 день')], ['3_days', copy.before(resolveLanguage(language) === 'en' ? '3 days' : resolveLanguage(language) === 'ro' ? '3 zile' : '3 дня')], ['1_week', copy.before(resolveLanguage(language) === 'en' ? '1 week' : resolveLanguage(language) === 'ro' ? '1 săptămână' : '1 неделю')]]} /></div>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            {step > 1 && step < 3 && <button type="button" onClick={() => setStep((value) => value + 1)} className="ml-auto flex-1 rounded-xl bg-amber-400 px-4 py-3.5 text-sm font-bold text-zinc-950 transition hover:bg-amber-300">{copy.next}</button>}
            {step === 3 && <button type="submit" disabled={busy} className="ml-auto flex-1 rounded-xl bg-amber-400 px-4 py-3.5 text-sm font-bold text-zinc-950 shadow-lg shadow-amber-500/15 transition hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-45">{busy ? copy.saving : editing ? copy.save : copy.create}</button>}
          </div>
        </div>
      </form>
    </div>
