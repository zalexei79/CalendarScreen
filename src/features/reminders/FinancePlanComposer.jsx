  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-stretch justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-3" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form onSubmit={submit} className={`flex h-[100dvh] w-full max-w-[460px] flex-col overflow-hidden rounded-none border shadow-[0_28px_90px_rgba(0,0,0,.55)] sm:h-auto sm:rounded-[28px] ${panel}`}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:max-h-[min(72dvh,560px)] sm:flex-none sm:px-6">
          ...
          {step === 1 && <section className="min-h-[250px]">
            ...
            <input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} className={`w-full rounded-xl border px-4 py-3.5 text-base outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-400/10 sm:text-sm ${field}`} placeholder={copy.placeholder} />
